const RAVEN_CACHE="raven-app-v62";
const RAVEN_SHELL=[
  "./index.html","./manifest.webmanifest","./icons/raven-icon.svg","./icons/raven-maskable.svg","./icons/raven-192.png","./icons/raven-512.png",
  "./css/raven.css?v=62-tamagotchi1","./data/bad-wuennenberg-pois.json?v=28sync1","./data/fuerstenberg-boundary.json?v=1",
  ...
  "./js/edge-tests.js?v=41edge-tests1","./js/game-tests.js?v=62-tamagotchi1","./js/pwa.js?v=62-tamagotchi1"
];
...
self.addEventListener("fetch",event=>{
  ...
  if(request.mode==="navigate"){
    event.respondWith(fetch(request).then(response=>{const copy=response.clone();caches.open(RAVEN_CACHE).then(cache=>cache.put("./index.html",copy));return response;}).catch(()=>caches.match("./index.html")));return;
  }
