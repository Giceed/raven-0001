...
const RAVEN_DAILY_COLLECT_TARGET=3;
...
function ravenVisualState(life=ravenLife,sleeping=isRavenSleeping(),shownMood=effectiveRavenMood()){
  if(sleeping)return{state:"sleeping",line:`${ravenProfile.name} schläft und sammelt Energie.`};
  if(life.hunger<25)return{state:"hungry",line:`${ravenProfile.name} hat Hunger und wartet auf Futter.`};
  if(life.energy<25)return{state:"tired",line:`${ravenProfile.name} ist müde und möchte schlafen.`};
  if(shownMood<40)return{state:"sad",line:`${ravenProfile.name} braucht gerade etwas Aufmerksamkeit.`};
  if(shownMood>=80)return{state:"happy",line:`${ravenProfile.name} ist glücklich und abenteuerlustig.`};
  return{state:"content",line:`${ravenProfile.name} beobachtet neugierig seine Umgebung.`};
}
function showRavenReaction(action,text,duration=1700){
  const habitat=document.getElementById("ravenHabitat"),bubble=document.getElementById("ravenReaction");if(!habitat||!bubble)return;
  clearTimeout(ravenReactionTimer);habitat.dataset.action=action;bubble.textContent=text;bubble.classList.add("active");
  ravenReactionTimer=setTimeout(()=>{delete habitat.dataset.action;bubble.classList.remove("active");ravenReactionTimer=null;},duration);
}
...
function restRaven(){
  ...
  saveRavenLife();setRavenLifeMessage(`${ravenProfile.name} schläft jetzt ungefähr ${minutes} Minute(n).`);renderRavenGamePanel();showRavenReaction("sleeping","Gute Nacht … 💤",2200);
}
function playWithRaven(){
  ...
  saveRavenLife();recordDailyTask("play");setRavenLifeMessage("Raven spielt begeistert – das kostet Energie und macht hungrig.");renderRavenGamePanel();showRavenReaction("playing","Nochmal! 💜");
}
...
function ravenDailyTaskDone(task,daily=ravenDaily){return task==="collect"?(daily.collect||0)>=RAVEN_DAILY_COLLECT_TARGET:(daily[task]||0)>0;}
function ravenDailyComplete(daily=ravenDaily){return ravenDailyTaskDone("feed",daily)&&ravenDailyTaskDone("play",daily)&&ravenDailyTaskDone("collect",daily);}
function recordDailyTask(task,amount=1){if(ravenTestMode)return;ravenDaily=loadRavenDaily();ravenDaily[task]=(ravenDaily[task]||0)+amount;const done=ravenDailyComplete(ravenDaily);if(done&&!ravenDaily.rewarded){ravenDaily.rewarded=true;grantRavenMissionItems({lieblingsfutter:1});if(typeof addXP==="function")addXP(20);setRavenLifeMessage("☀️ Tagesaufgaben geschafft: Lieblingsfutter und 20 XP!");}localStorage.setItem("ravenDaily",JSON.stringify(ravenDaily));renderRavenGamePanel();}
...
function renderRavenGamePanel(){
  ...
  const sleeping=isRavenSleeping(),visual=ravenVisualState(ravenLife,sleeping,shownMood),habitat=document.getElementById("ravenHabitat"),stateLine=document.getElementById("ravenStateLine");if(habitat)habitat.dataset.state=visual.state;if(stateLine)stateLine.textContent=visual.line;
  ...
  const avatar=document.getElementById("ravenAvatar");if(avatar){const status=ravenMoodStatus();avatar.textContent=sleeping?"🐦‍⬛💤":status==="Braucht dich"?"🐦‍⬛❗":ravenLife.energy<30?"🐦‍⬛💤":ravenLife.hunger<30?"🐦‍⬛🍽️":shownMood>80?"🐦‍⬛✨":"🐦‍⬛";avatar.dataset.mood=sleeping?"Schläft":status;}
  ...
  ravenDaily=loadRavenDaily();const dailyTasks=[{id:"feed",label:"Raven füttern"},{id:"play",label:"Mit Raven spielen"},{id:"collect",label:`3 Aktivitätspunkte sammeln · ${Math.min(ravenDaily.collect||0,RAVEN_DAILY_COLLECT_TARGET)}/${RAVEN_DAILY_COLLECT_TARGET}`}],dailyDone=dailyTasks.filter(task=>ravenDailyTaskDone(task.id,ravenDaily)).length;
  const dailyState=document.getElementById("dailyState"),dailyList=document.getElementById("dailyList");if(dailyState)dailyState.textContent=ravenDaily.rewarded?"✓ FERTIG":`${dailyDone}/3`;if(dailyList)dailyList.innerHTML=dailyTasks.map(task=>{const done=ravenDailyTaskDone(task.id,ravenDaily);return `<div class="daily-row ${done?"done":""}"><span>${done?"✓":"○"}</span>${task.label}</div>`;}).join("");
