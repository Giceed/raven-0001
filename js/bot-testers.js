...
function makeRavenBot(definition,index){const start=[51.5157+(index-1)*.00012,8.741+(index-1)*.00012];return {...definition,lat:start[0],lon:start[1],start,targets:getRavenBotPoints(),targetIndex:0,target:null,segments:[],results:[],trail:[{lat:start[0],lon:start[1]}],routeChecks:0,routeFallbacks:0,running:false,finished:false,state:"Bereit"};}
...
function recordRavenBotTrail(bot){
  if(bot.id==="car"&&!bot.carStopped)return;const last=bot.trail[bot.trail.length-1];if(!last||haversineDistance(last.lat,last.lon,bot.lat,bot.lon)>=18){bot.trail.push({lat:bot.lat,lon:bot.lon});if(bot.id===activeRavenBotId)redrawRavenBotFog();}
}

function renderRavenBotCards(){
  const grid=document.getElementById("botGrid");if(!grid)return;
  grid.innerHTML=ravenBots.map(bot=>{const passed=bot.results.filter(result=>result.ok).length,total=bot.targets.length,percent=total?Math.round(passed/total*100):0;return `<div class="bot-card ${bot.id} ${activeRavenBotId===bot.id?"viewed":""}"><div class="bot-name">${bot.icon} ${bot.name} · ${bot.label}</div><div class="bot-state">${escapeHTML(bot.state)}</div><div class="bot-progress"><i style="width:${percent}%"></i></div><div class="bot-score">${passed}/${total} bestanden · ${bot.routeChecks} Wegrouten</div><div class="bot-card-actions"><button class="run" onclick="startRavenBot('${bot.id}')">${bot.finished?"Neu starten":"Starten"}</button><button class="halt" onclick="stopRavenBot('${bot.id}')">Stoppen</button><button class="view" onclick="showRavenBotFog('${bot.id}')">Ansehen</button></div></div>`;}).join("");
}
...
function advanceRavenBot(bot,seconds){
  ...
  bot.marker?.setLatLng([bot.lat,bot.lon]);recordRavenBotTrail(bot);if(bot.segmentIndex>=bot.segments.length)finishRavenBotPoint(bot);
}
...
function resetRavenBot(bot){bot.running=false;bot.finished=false;bot.lat=bot.start[0];bot.lon=bot.start[1];bot.targetIndex=0;bot.target=null;bot.segments=[];bot.results=[];bot.trail=[{lat:bot.start[0],lon:bot.start[1]}];bot.routeChecks=0;bot.routeFallbacks=0;bot.state="Startbereit";if(bot.routeLayer){ravenBotLayer.removeLayer(bot.routeLayer);bot.routeLayer=null;}createRavenBotMarker(bot);drawRavenBotPois();redrawRavenBotFog();}
