...
function closeRavenJumper(){stopRavenJumper();document.getElementById("jumperOverlay")?.classList.remove("open","game-over");document.body.style.overflow="";}
function startRavenJumper(){
  ...
  document.getElementById("jumperOverlay")?.classList.remove("game-over");document.getElementById("jumperMessage").textContent="Flieg, Raven!";document.getElementById("jumperStartButton").textContent="LÄUFT";
...
function ravenJumperLoop(now){
  ...
  if(p.vy>0)for(const platform of ravenJumper.platforms){if(oldBottom<=platform.y+5&&p.y+p.h>=platform.y&&p.x+p.w>platform.x&&p.x<platform.x+platform.w){p.y=platform.y-p.h;p.vy=-10.2;break;}}
...
function finishRavenJumper(){const score=Math.floor(ravenJumper.score/10),old=jumperHighscore(),best=Math.max(old,score);localStorage.setItem(RAVEN_JUMPER_KEY,String(best));ravenJumper.running=false;const mood=Math.min(12,Math.max(2,Math.floor(score/20)+2));if(typeof ravenLife!=="undefined"){ravenLife.mood=clampRavenNeed(ravenLife.mood+mood);saveRavenLife();recordDailyTask("play");renderRavenGamePanel();}document.getElementById("jumperMessage").textContent=score>old?`Neuer Rekord: ${score}! Stimmung +${mood}`:`${score} Punkte · Stimmung +${mood}`;document.getElementById("jumperStartButton").textContent="NOCHMAL";document.getElementById("jumperOverlay")?.classList.add("game-over");renderJumperHighscore();}
...
window.addEventListener("DOMContentLoaded",()=>{renderJumperHighscore();document.querySelectorAll("[data-jumper]").forEach(button=>{const direction=button.dataset.jumper;["pointerdown"].forEach(type=>button.addEventListener(type,event=>{event.preventDefault();setJumperDirection(direction,true);}));["pointerup","pointercancel","pointerleave"].forEach(type=>button.addEventListener(type,()=>setJumperDirection(direction,false)));});});
